...
- All API keys (Firecrawl, Anthropic, ElevenLabs) must be server-side only — never exposed to the frontend
- Scraped website content from Firecrawl must be sanitized before rendering in the frontend to prevent XSS
- Product website content passed to LLM as context must be sanitized to prevent prompt injection
- No user accounts in v1 — no auth, no sensitive user data
- Audio files are served from the backend — no direct filesystem paths exposed to clients
- Rate limit the `/api/generate` endpoint to prevent API credit abuse
